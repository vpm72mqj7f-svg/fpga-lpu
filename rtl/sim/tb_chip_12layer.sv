...
module tb_chip_12layer;
    ...
    // KV cache preload (for prefill KV → HBM path)
    ...
    task preload_ffn();
        for (int r = 0; r < 4; r++) begin
            @(posedge clk); gate_w_wr_en<=1; gate_w_wr_row<=r[$clog2(4)-1:0]; gate_w_wr_beat<=0;
            gate_w_wr_data<={4'h4,4'h0,4'h0,4'h0}; @(posedge clk); gate_w_wr_en<=0;
            @(posedge clk); gate_w_wr_en<=1; gate_w_wr_row<=r[$clog2(4)-1:0]; gate_w_wr_beat<=1;
            gate_w_wr_data<=16'h0; @(posedge clk); gate_w_wr_en<=0;
            @(posedge clk); up_w_wr_en<=1; up_w_wr_row<=r[$clog2(4)-1:0]; up_w_wr_beat<=0;
            up_w_wr_data<={4'h4,4'h0,4'h0,4'h0}; @(posedge clk); up_w_wr_en<=0;
            @(posedge clk); up_w_wr_en<=1; up_w_wr_row<=r[$clog2(4)-1:0]; up_w_wr_beat<=1;
    ...
    //=========================================================================
    // Module-level arrays for Icarus compatibility (no unpacked task ports)
    //=========================================================================
    logic [31:0] task_in_vec  [8];
    logic [31:0] task_out_vec [8];
    int          task_latency;

    //=========================================================================
    // Run one layer: feed token, wait for output, store in task_out_vec
    //=========================================================================
    task run_layer(input int layer_id);
        integer cyc;
        int start_cycle, end_cycle;
        begin
            for (int d = 0; d < HIDDEN; d++) a_flat[d*32+:32] <= task_in_vec[d];
            ...
            // Wait for output
            task_latency = 0;
            task_out_vec[0] = '0; task_out_vec[1] = '0; task_out_vec[2] = '0; task_out_vec[3] = '0;
            task_out_vec[4] = '0; task_out_vec[5] = '0; task_out_vec[6] = '0; task_out_vec[7] = '0;
            while (!valid_out) begin
                @(posedge clk);
                task_latency = task_latency + 1;
                if (task_latency > 5000) begin
                    $error("Layer %0d: TIMEOUT after %0d cycles", layer_id, task_latency);
                    $fatal;
                end
            end
            #1;
            end_cycle = $time / 10;
            task_latency = end_cycle - start_cycle;
            task_out_vec[0] = y0; task_out_vec[1] = y1; task_out_vec[2] = y2; task_out_vec[3] = y3;
            task_out_vec[4] = y4; task_out_vec[5] = y5; task_out_vec[6] = y6; task_out_vec[7] = y7;
        end
    endtask

    //=========================================================================
    // Main test (module-level vars for Icarus compatibility)
    //=========================================================================
    ...
    initial begin
        ...
        for (int chip = 0; chip < NUM_CHIPS; chip++) begin
            ...
            for (int layer = 0; layer < LAYERS_PER_CHIP; layer++) begin
                ...
                // Copy token_vec to task_in_vec
                for (int d = 0; d < 8; d++) task_in_vec[d] = token_vec[d];
                run_layer(global_layer);
                // Copy task output to next_vec
                for (int d = 0; d < 8; d++) next_vec[d] = task_out_vec[d];
                layer_lat = task_latency;
