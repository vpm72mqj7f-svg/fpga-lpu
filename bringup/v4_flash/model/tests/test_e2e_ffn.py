#!/usr/bin/env python3
"""
E2E test harness for DeepSeek V4-Flash FFN pipeline.

Tests the complete FPGA FFN forward path:
  gate projection -> up projection -> SiLU activation -> down projection
  with MoE TOP_K routing and shared expert.

Matches the RTL microarchitecture in v4_flash_ffn_engine.sv.

Usage:
    python3 tests/test_e2e_ffn.py            # run all tests
    python3 tests/test_e2e_ffn.py --quick    # skip full-scale timing (CI)
    pytest tests/test_e2e_ffn.py -v          # pytest runner
"""
...
# -- Architecture constants (V4-Flash production) -----------------------------

HIDDEN      = 7168
INTER       = 3072
NUM_EXPERTS = 385       # 384 routed + 1 shared
TOP_K       = 6
...
def test_fp8_roundtrip():
    """Validate FP8 E4M3 encode/decode on known values."""
    test_section('FP8 E4M3 Roundtrip Accuracy')
...
_SMALL_HIDDEN = 64
_SMALL_INTER  = 32
_SMALL_EXPERTS = 7   # 6 routed + 1 shared
_SMALL_TOP_K  = 6
...
def _build_small_pipeline(seed: int = 42) -> FFNPipeline:
    ...
    # Routed experts 0..5
    for eid in range(_SMALL_EXPERTS - 1):
...
def test_full_ffn_small():
    """Complete FFN forward pass with small dimensions."""
    test_section(f'Full FFN Inference (HIDDEN={_SMALL_HIDDEN}, INTER={_SMALL_INTER}, '
                 f'{_SMALL_EXPERTS - 1}+1 experts)')
    ...
    check(f'  loaded {pipe.num_loaded_experts} routed experts',
          pipe.num_loaded_experts == _SMALL_EXPERTS - 1)
    check('  shared expert loaded',
          pipe.shared_expert is not None)
    ...
    check('output: selected experts valid',
          all(0 <= eid < _SMALL_EXPERTS - 1 for eid in pipe.last_selected_experts))
    check('output: no NaN',
...
def test_single_expert():
    ...
    for eid in range(_SMALL_EXPERTS):  # 0..5 routed + 6 shared
        ...
        try:
            ...
            check(f'{name}: shape={out_fp8.shape}, range=[{out_f32.min():.3f}, {out_f32.max():.3f}]',
                  ok)
...
def test_edge_nan_input():
    ...
    # Note: module-level wrappers (OLD convention) preserve NaN: NaN → 0x7F → NaN
    # The FP8_E4M3 class methods (NEW convention) map NaN → 0.
    activ_raw_fp8 = float_to_fp8(x)
    decoded = fp8_to_float(activ_raw_fp8)
    nan_count = np.sum(np.isnan(decoded))
    # OLD wrappers preserve NaN through encode/decode roundtrip (NaN → 0x7F → NaN)
    check(f'FP8 encode/decode: NaN preserved ({nan_count} NaNs in decoded, as expected for OLD wrapper)',
          nan_count >= 3,  # at least 3 NaNs preserved (or more due to encoding error propagation)
...
def test_verification_apis():
    ...
    # With 0.01 shift, relative error should be small but non-zero
...
def test_full_scale_timing(skip_timing: bool = False):
    """Run with V4-Flash production dimensions -- timing only."""
    ...
    if skip_timing:
        skip('full-scale timing test',
             'use --full to run (loads 2 experts + shared = ~198 MB FP8 weights)')
        return

    print(f'  Building pipeline with {NUM_EXPERTS} experts '
          f'({NUM_EXPERTS - 1} routed + 1 shared)...')
    print(f'  Weight shapes: gate_w [{HIDDEN}, {INTER}] = '
          f'{HIDDEN * INTER * 1 / 1024 / 1024:.1f} MB FP8')

    t0 = time.perf_counter()
    pipe = FFNPipeline(hidden=HIDDEN, inter=INTER,
                       num_experts=NUM_EXPERTS, top_k=TOP_K)

    # Load only 2 routed experts + shared (enough for correctness; avoids OOM)
    n_route = min(2, NUM_EXPERTS - 1)
    ...
    check(f'pipeline built ({n_route} routed + 1 shared) in {t_build:.2f} s',
          t_build > 0)
    ...
    print(f'  Per-expert time estimate: {t_full * 1000 / (n_route + 1):.2f} ms')

    # HW target: ~4.1 ms for 6 experts at 128 lanes x 500 MHz
    hw_target = 240.0
    print(f'\n  HW target (FPGA 128 lanes x 500 MHz): ~{hw_target:.0f} tok/s')
    print(f'  Python sim vs HW: {tok_s / hw_target:.4f}x  (not performance-competitive)')
...
def main():
    parser = argparse.ArgumentParser(
        description='DeepSeek V4-Flash FFN E2E test harness')
    parser.add_argument('--quick', action='store_true',
                        help='Skip full-scale timing test')
    parser.add_argument('--full', action='store_true',
                        help='Run full-scale timing test (expensive: builds ~198 MB of weights)')
    args = parser.parse_args()

    print(_bold('\n+==============================================================+'))
    print(_bold('|   DeepSeek V4-Flash FFN Pipeline -- E2E Test Harness'))
    print(_bold('|   Target: v4_flash_ffn_engine.sv (RTL reference)'))
    print(_bold('+==============================================================+'))

    # Architecture summary
    print(f'\n  Architecture: HIDDEN={HIDDEN}, INTER={INTER}, '
          f'NUM_EXPERTS={NUM_EXPERTS} ({NUM_EXPERTS - 1}+1 shared), TOP_K={TOP_K}, FP8')
    print(f'  Test device:  {os.name}')

    # -- Run tests ---------------------------------------------------------

    tests = [
        ('FP8 Roundtrip Accuracy',       test_fp8_roundtrip),
