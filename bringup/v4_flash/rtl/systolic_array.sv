//=============================================================================
// systolic_array.sv — V4-Flash 128-Lane Systolic Array Controller
// Target: Intel Stratix 10 MX (1SM21BHU2F53E1VG), Quartus Pro 26.1
//
// Architecture:
//   128 fp8×fp8→fp16 MAC lanes. Activation-stationary within each row.
//   Weights stream from HBM2 via hbm2_weight_reader. Partial dot products
//   accumulate over K cycles per row, then a 128:1 pipelined adder tree
//   reduces lane results into a single row output.
//
// FSM: IDLE → WEIGHT_PRELOAD → STREAM → DRAIN → REDUCE → STORE → NEXT_ROW → DONE
//
// Parameterized for three projection types:
//   Gate:  INPUT_DIM=7168, OUTPUT_DIM=3072  (56 cycles/row, 3072 rows)
//   Up:    INPUT_DIM=7168, OUTPUT_DIM=3072  (56 cycles/row, 3072 rows)
//   Down:  INPUT_DIM=3072, OUTPUT_DIM=7168  (24 cycles/row, 7168 rows)
//
...
//
// Resource (production, DSP_LANES=128):
//   DSP blocks:  128 (multiply only, accumulation in fabric)
//   M20K:        < 4 (reduction pipeline registers)
//   ALMs:        ~12k (accumulators + reduction + control)
//=============================================================================

module systolic_array #(
    parameter int INPUT_DIM  = 7168,     // activation / row dimension
    parameter int OUTPUT_DIM = 3072,     // number of output rows
    parameter int DSP_LANES  = 128,      // column parallelism
    ...
    // Derived Parameters
    //=========================================================================
    localparam int CYCLES_PER_ROW   = INPUT_DIM / DSP_LANES;
    ...
    localparam int REDUCE_STAGES    = $clog2(DSP_LANES);  // 7 stages for 128 lanes
    ...
    typedef enum logic [3:0] {
        ...
        S_REDUCE,           // Pipelined reduction: 128 lanes → 1 result
    ...
    //=========================================================================
    // preload_next_row holds the row index being preloaded for the NEXT compute
    ...
    //=========================================================================
    // MAC Lanes — Generate 128 fp8×fp8 multiply-accumulate instances
    //
    ...
    //=========================================================================
    // Pipelined Reduction — 128:1 Adder Tree (7 registered stages)
    //
    ...
    //=========================================================================
    localparam int N_STAGES   = REDUCE_STAGES;   // 7 for 128 lanes
    localparam int N_LANES    = DSP_LANES;        // 128
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // This overlaps with the previous row's compute/store.
                // On first entry: request row 0 weights.
                // On subsequent entries: weights for the next row are already
                // preloaded (requested during previous row's STREAM).
                // ==============================================================
                S_WT_PRELOAD: begin
                    // Issue preload request for current_row if not already active
                ...
                // ==============================================================
                S_STREAM: begin
                    // activ_ready and weight_ready are asserted → data flows
                ...
                // ==============================================================
                // REDUCE — Pipelined 128:1 adder tree
                //
                ...
                S_STORE: begin
                    ...
                    if (!result_valid && result_valid_dbuf[result_sel_read]) begin
                        ...
                        // (will be used when the NEXT reduction completes)
