...
// Resource: 1 M20K block (LUT), ~128 DSP blocks (fp16 multiply)
// Throughput: 128 elements/cycle after pipeline fill
// =============================================================================

module silu_activation #(
    parameter int DATA_W    = 16,     // fp16 input/output
    parameter int NUM_ELEMS = 128,    // parallel elements per cycle
    ...
    function automatic logic [DATA_W-1:0] compute_sigmoid_fp16(
        ...
        // Encode as fp16
        return $realtobits(sig)[15:0];  // simplified: use verilog real for LUT init
    endfunction
    ...
    initial begin
        ...
        for (int i = 1; i < 16; i++)
            sigmoid_lut['h80 + i - 1] = 16'h3000;  // σ(-0.13) ≈ 0.468
