...
//
// DSP Budget: 128 lanes × 1 DSP per multiply = 128 DSP blocks
//   S10 MX has 3,960 DSPs (3.2% utilization)
//
...
module fp8_mac #(
    parameter int NUM_LANES = 128,    // systolic array width
        ...
        for (gi = 0; gi < NUM_LANES; gi++) begin : gen_lane
            ...
            always_comb begin
                ...
                if (s1_prod_zero[gi]) begin
                    ...
                    // Guard = bit (right_shift-1) of mant before shift
                    // Sticky = OR of bits [right_shift-2:0]
                    mant_shifted = ({1'b0, s2_mant_norm} >> right_shift);

                    // Compute round bit from the shifted-out portion
                    // Extract the bits shifted out:
                    //   shifted_out = s2_mant_norm & ((1 << right_shift) - 1)
                    // guard = MSB of shifted_out
                    // sticky = OR of remaining bits
            ...
            always_comb begin
                ...
                // Mantissa with implicit 1 prepended (for normal numbers)
                //   Normal: {1'b1, mant[9:0]}  (11 bits)
                //   Subnormal/Zero: {1'b0, mant[9:0]}  (11 bits, no implicit 1)
                //   Extended to 12 bits with guard bit for rounding
                s2_mant_a_aligned = 12'b0;
                s2_mant_b_aligned = 12'b0;

                if (s2_exp_diff >= 0) begin
                    // a has larger or equal exponent; shift b right
                    // Extended mant_a: 12 bits = {implicit, explicit[9:0], guard}
                    s2_mant_a_aligned = {(s2_a_exp == 5'b0 ? 1'b0 : 1'b1), s2_a_mant, 1'b0};
                    if (s2_exp_diff <= 6'sd12) begin
                        // b mantissa: include implicit bit, then extend, then shift right
                        s2_mant_b_aligned = ({(s2_b_exp == 5'b0 ? 1'b0 : 1'b1), s2_b_mant, 1'b0}
                                            >> s2_exp_diff[3:0]);
                    end else begin
                        // b shifted out completely
                        s2_mant_b_aligned = 12'b0;
                    end
                end else begin
            ...
            always_comb begin
                ...
                // Actually: we want the mantissa normalized so that the leading 1
                // is at bit position 10 (implicit 1 for FP16).
                // s2_mant_sum is 13 bits [12:0]. The guard bit is bit 0.
                // The implicit 1 should be at bit 11 of the aligned mantissa.
                // After addition, implicit 1 is at s2_sum_lod position.
                // We need to shift so that the implicit 1 is at bit 10.
                //
                // For normal result: mant_norm = (sum >> (lod - 10)) & 0x3FF
                // exp_final = max(s2_a_exp, s2_b_exp) + (lod - 11)
                // Wait, careful here. The aligned mantissas were:
                //   {implicit, explicit[9:0], guard} = 12 bits
                // So implicit is at bit 11, guard at bit 0.
                // After add/sub, we have a 13-bit sum [12:0].
                // The implicit 1 of the result is at lod position.
                // We want it at bit 10 for FP16 output.
                //
                // shift = lod - 10 (signed)
                // If shift > 0: shift right (overflow, increment exponent)
                // If shift < 0: shift left (cancellation, decrement exponent)
                // If shift == 0: no shift needed

                automatic logic signed [4:0] norm_shift;
                automatic logic [FP16_EXP_W-1:0] larger_exp;
                automatic logic [FP16_EXP_W-1:0] exp_unbiased_adj;

                larger_exp = (s2_exp_diff >= 0) ? s2_a_exp : s2_b_exp;

                // lod ranges 0..12. We want implicit at bit 10, so shift = lod - 10
                // But careful: lod gives the bit position of the MSB in s2_mant_sum
                // s2_mant_sum bit 11 corresponds to the pre-add implicit 1.
                // So: shift = s2_sum_lod - 11 + 1? Let me just be explicit.
                // s2_mant_sum[12] is carry bit; s2_mant_sum[11] is the original implicit position.
                // For a sum where no carry: lod = 10 or 11.
                // For a sum with carry (overflow): lod = 11 or 12.
                ...
                if (s2_mant_sum == 13'b0) begin
                    ...
                    // mant_final = sum >> norm_shift, with rounding
                    // exp_final = larger_exp + norm_shift

                    // RNE rounding: guard = bit (norm_shift-1), sticky = OR of lower bits
                    ...
                    if (s2_mant_final[10] && !mant_shifted[10]) begin
                        ...
                        s2_mant_final = 11'b0;  // mantissa rolls over to implicit 1 only
                    end else begin
                        s2_exp_final = larger_exp + norm_shift[3:0];
                    end
                end else if (norm_shift < 0) begin
                    // Need to shift left (cancellation)
                    // shift left by |norm_shift|; exponent decreases
    ...
    //
    // Resource Estimates (per lane, per module total with NUM_LANES=128):
    //   - DSP blocks:    1 (mantissa 4×4→8 multiply) × 128 = 128 DSPs
    //   - ALMs:          ~80 ALMs/lane (LOD + shifter + FP16 add) = ~10,240 ALMs
    //   - Registers:     ~80 FFs/lane (pipeline + accumulate state) = ~10,240 FFs
    //   - S10 MX total:  933,120 ALMs, 3,960 DSPs → ~1.1% ALMs, ~3.2% DSPs
    //
